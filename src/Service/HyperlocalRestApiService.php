...
namespace Webkul\MPHyperlocal\Service;
...
class HyperlocalRestApiService
{
    /**
     * @var Client
     */
    ...
    private function getAdminAccess(): void
    {
        $body = \json_encode([
            ...
            'grant_type' => 'password',
            'scopes' => $this->config->get('WebkulMPHyperlocal.config.scope'),
            'username' => $this->config->get('WebkulMPHyperlocal.config.username'),
            'password' => $this->config->get('WebkulMPHyperlocal.config.password')
        ]);
